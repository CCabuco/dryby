...
export default function SignUpScreen() {
  ...
  const createAccount = async () => {
    if (!FIREBASE_CONFIG_READY) {
      setSubmitError(
        "Account creation is unavailable until Firebase credentials are added to the app's local .env file."
      );
      return;
    }
    ...
    try {
      ...
      if (error?.code === "auth/email-already-in-use") {
        ...
        message = "Network error while creating the account. Check your connection and try again.";
      } else if (error?.code === "auth/api-key-not-valid") {
        message = "Firebase API key is invalid. Update the app's Firebase credentials.";
      } else if (error?.code === "auth/operation-not-allowed") {
        message = "Email/password sign-up is not enabled in Firebase Authentication yet.";
      }
      ...
      >
        ...
        >
          ...
          <View style={styles.card}>
            ...
            {!FIREBASE_CONFIG_READY && step === TOTAL_STEPS ? (
              <Text style={styles.warningText}>
                Firebase credentials are missing, so account creation is disabled on this device
                until the local `.env` file is configured.
              </Text>
            ) : null}
            ...
            <View style={styles.buttonArea}>
              ...
              <TouchableOpacity
                style={[
                  ...
                  (isSubmitting || (step === TOTAL_STEPS && !FIREBASE_CONFIG_READY)) &&
                    styles.disabledButton,
                ]}
                onPress={nextStep}
                disabled={isSubmitting || (step === TOTAL_STEPS && !FIREBASE_CONFIG_READY)}
              >
